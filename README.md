...
- AI Summary: OpenAI SDK + DashScope OpenAI 兼容模式，支持 OpenAI 与阿里云 Qwen (`DASHSCOPE_API_KEY` 或 `OPENAI_API_KEY` 可选)
...
- `OPENAI_MODEL=gpt-4o-mini` 或 `DASHSCOPE_MODEL=qwen3-vl-8b-thinking` 或 `OPENAI_BASE_URL` / `DASHSCOPE_BASE_URL`

仓库根目录已附带 `render.yaml` 作为 Blueprint 示例。根据阿里云百炼 OpenAI 兼容文档，DashScope 兼容模式的 `base_url` 为 `https://dashscope.aliyuncs.com/compatible-mode/v1`，模型名可直接使用 `qwen3-vl-8b-thinking`。
