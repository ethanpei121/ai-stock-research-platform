...
function QuotePriceCell({ symbol, quoteSnapshots }: { symbol: string; quoteSnapshots: RecommendationQuoteMap }) {
  ...
  return <span className="font-mono text-sm text-slate-900">{formatCurrency(snapshot.data.price, snapshot.data.currency)}</span>;
}
...
function QuoteChangeCell({ symbol, quoteSnapshots }: { symbol: string; quoteSnapshots: RecommendationQuoteMap }) {
  ...
  return (
    <span className={`inline-flex rounded-md px-2 py-1 font-mono text-xs font-semibold ${tone.badgeClassName}`}>
...
function scoreLabel(row: RecommendationRow) {
  if (!row.scorecard) {
    return <span className="rounded-md bg-slate-100 px-2 py-1 text-[11px] font-medium text-slate-700">固定观察池</span>;
  }

  return (
    <div className="flex flex-col gap-1">
      <span className="font-mono text-sm font-semibold text-slate-900">{row.scorecard.total.toFixed(1)}</span>
      <span className="rounded-md bg-slate-100 px-2 py-1 text-[11px] font-medium text-slate-700">{row.scorecard.label}</span>
    </div>
  );
}
      ...
      <div className="rounded-lg border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-slate-200 px-5 py-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-3">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500">Market Dashboard</p>
              <h2 className="mt-2 text-xl font-semibold text-slate-900">推荐股票池</h2>
            </div>
            <p className="max-w-3xl text-sm leading-6 text-slate-500">
              首页默认展示固定观察池，并同步回补每只股票的市场快照。点击任意股票或使用顶部搜索框，即可从右侧抽屉打开个股 AI 分析。只有点击“刷新实时推荐”时，才会请求后端进行动态重排。
            </p>
          </div>
          ...
          <div className="flex flex-col items-start gap-3 lg:items-end">
            ...
            <button
              type="button"
              className="inline-flex items-center gap-2 rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 disabled:cursor-progress disabled:opacity-60"
        ...
        <div className="grid gap-4 border-b border-slate-200 px-5 py-4 md:grid-cols-[220px_220px_minmax(0,1fr)] md:items-end">
          <label className="grid gap-2 text-sm font-medium text-slate-700">
            行业
            <select
              value={selectedCategory}
              className="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none ring-0 transition focus:border-slate-400"
        ...
        {refreshError ? <div className="border-b border-red-200 bg-red-50 px-5 py-3 text-sm text-red-600">{refreshError}</div> : null}

        {section.status === "loading" ? (
          <div className="px-5 py-10 text-sm text-slate-500">推荐股票池正在加载。</div>
        ) : null}

        {section.status === "error" ? <div className="px-5 py-10 text-sm text-red-600">{section.error}</div> : null}
        ...
        {data ? (
          rows.length > 0 ? (
            <>
              <div className="hidden overflow-x-auto lg:block">
                <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
                  <thead className="sticky top-0 z-[1] bg-slate-50 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                    <tr>
                      ...
                      return (
                        <tr
                          ...
                          className={`cursor-pointer align-top transition focus:outline-none ${
                            isActive ? "bg-slate-100" : "hover:bg-slate-50 focus:bg-slate-50"
                          }`}
                        ...
                        >
                          <td className="px-5 py-4">
                            <div className="space-y-2">
                              ...
                              <div className="flex flex-wrap gap-2">
                                <span className="rounded-md bg-slate-100 px-2 py-1 text-[11px] font-medium text-slate-700">{row.market}</span>
                                <span className="rounded-md bg-slate-100 px-2 py-1 text-[11px] font-medium text-slate-700">{row.region}</span>
                              </div>
                            </div>
                          </td>
                          <td className="px-5 py-4">
                            <div className="space-y-1.5">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {row.tags.slice(0, 3).map((tag) => (
                                  <span key={`${row.symbol}-${tag}`} className="rounded-md bg-slate-100 px-2 py-1 text-[11px] font-medium text-slate-700">
                                    {tag}
                                  </span>
                                ))}
                              </div>
                          ...
                          <td className="px-5 py-4">{scoreLabel(row)}</td>
                          <td className="px-5 py-4">
                            <div className="space-y-2">
                              <p className="text-sm leading-6 text-slate-700">{row.rationale}</p>
                              <div className="flex flex-wrap gap-2">
                                {row.styles.map((style) => (
                                  <span key={`${row.symbol}-${style}`} className="rounded-md bg-slate-100 px-2 py-1 text-[11px] font-medium text-slate-700">
                                    {style}
                                  </span>
                                ))}
                              </div>
                ...
                {rows.map((row) => {
                  ...
                  return (
                    <button
                      key={`${row.groupId}-${row.symbol}`}
                      type="button"
                      className={`grid w-full gap-4 px-5 py-4 text-left transition ${isActive ? "bg-slate-100" : "hover:bg-slate-50"}`}
                      onClick={() => onOpenSymbol(row.symbol)}
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <div className="flex items-center gap-3">
                            ...
                            <span className="rounded-md bg-slate-100 px-2 py-1 text-[11px] font-medium text-slate-700">{row.category}</span>
                          </div>
            ...
            <div className="px-5 py-10 text-sm text-slate-500">当前筛选条件下没有可展示的股票。</div>
          )
        ) : null}
      </div>
    </section>
